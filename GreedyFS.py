...
class Node:
    ...
    def readGrid (filename):
        ...
        with open(filename) as f:
            ...
            return grid
    #this grid for the put number of distance from Goal   
    grid = readGrid("textFile.txt")
    #this grid2 for put path 
    ...
    while find:
        ...
        for value, op in enumerate(openList):
            ...
            #distance calurate
            dist_node_goal = distance(op, orgGoal)
            #change the number on grid to distance value
            modifyGrid(grid2,op,dist_node_goal)

            #I just push the distance
            heappush(sortlowq, dist_node_goal)
        ...
        for x in openList:
            ...
            if value_of_sortlest == distance(x, orgGoal):
                y = x
                openList.insert(0,x)
        ...
        if grid2[y[0]][y[1]] == grid2[orgGoal[0]][orgGoal[1]]:
            modifyGrid(grid, y, "G")
            find = not find
            print "Success"
        #if value_of_sortlest == 0:
        #    find = not find
        #    print "Success"
